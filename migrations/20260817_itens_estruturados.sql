...
alter table public.ocorrencias
  add column if not exists armas_itens jsonb not null default '[]'::jsonb,
  add column if not exists municoes_itens jsonb not null default '[]'::jsonb,
  add column if not exists itens_recuperados jsonb not null default '[]'::jsonb;

alter table public.ocorrencias
  drop constraint if exists ocorrencias_armas_itens_array,
  drop constraint if exists ocorrencias_municoes_itens_array,
  drop constraint if exists ocorrencias_itens_recuperados_array;

alter table public.ocorrencias
  add constraint ocorrencias_armas_itens_array check (jsonb_typeof(armas_itens) = 'array'),
  add constraint ocorrencias_municoes_itens_array check (jsonb_typeof(municoes_itens) = 'array'),
  add constraint ocorrencias_itens_recuperados_array check (jsonb_typeof(itens_recuperados) = 'array');
...
begin
  ...
  update public.ocorrencias set
    ...
    entorpecentes=v.entorpecentes,maconha=v.maconha,cocaina=v.cocaina,pasta_base=v.pasta_base,crack=v.crack,skank=v.skank,
...
as $function$
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'itens_recuperados',o.itens_recuperados,'entorpecentes',o.entorpecentes,
